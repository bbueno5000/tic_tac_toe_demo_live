var Ttt = (function (Ttt) {
    ...
    /**
     * A convenient wrapper around a board and whose turn it is.
     * Also holds the game's state going back through time.
     * Ignoring the history, the current board state will tell you
     * whose turn it is assuming the game has been played according to the rules,
     * but this gives us some convenience and lets us bend the rules if we want to.
     * 
     * @param {any} board
     * @param {any} turn
     * @param {any} history
    ...
    /**
     * Boards are simply integers.
     * The first two bits encode the piece at square 0 (top left),
     * the next two bits are square 1, etc.
     * Squares are 0-8 left to right, top to bottom.
     * If a square's bits are both 0, that square is empty.
     * If the less significant bit is set, it's occupied.
     * The occupying player is determined by the more significant bit:
     * X if not set or O if set.
     */
    function newBoard() {
        return 0;
    }
    ...
    /**
    * A simple, synchronous driver for playing games.
    ...
    */
    function play(x, o) {
        ...
        do {
            ...
            if (move < 0 || move >= 9 || game.getPiece(move) !== 0) {
                throw new Error(
                    "AI chose invalid move " + move.toString() + " in " + game.toString()
                );
            }
            game.move(move);
    ...
    function winner(board, extra) {
        // extra gets its line property set if a player has won.
        // It's used to draw an appropriate line indicating the win.
        extra = extra || {};
